package lanou.foodpies.tools.sqltools;
...
import lanou.foodpies.beans.HistorySqlData;
import lanou.foodpies.values.DBValues;
...
public class DBTool {
                    ...
                    public void onQuery(ArrayList<HistorySqlData> historySqlData) {
                        ...
                        for (int i = 0; i < historySqlData.size(); i++) {
                            ...
                            if (string.equals(history)) {
                                deleteHistoryByCondition(HistorySqlData.class, history);
                            }
                        }
                        // 2. 当数据库数据已经存满 10条数据,删除最旧一条
                        if (historySqlData.size() >= 10){
                            String oldText = historySqlData.get(0).getHistoryStr();
                            deleteHistoryByCondition(HistorySqlData.class, oldText);
                        }
                        // 3. 将数据 存入数据库
    ...
    /** 按条件 删除某一条 收藏数据 */
    public void deleteCollectionByCondition(Class<CollectionSqlDataBean> collectionSqlDataClass, String username, String getLink) {
    ...
    private class DeleteCollectionDataByCondRunnable implements Runnable {
        private Class<CollectionSqlDataBean> collectionSqlDataClass;
        private String username;
        private String link;
        public DeleteCollectionDataByCondRunnable(Class<CollectionSqlDataBean> collectionSqlDataClass, String username, String getLink) {
    ...
    public void queryCollectionDataByUsername( String username, OnQueryListener<CollectionSqlDataBean> onQueryListener) {
    ...
    private class QueryCollectionRunnable implements Runnable{
        ...
        @Override
        public void run() {
            ArrayList<CollectionSqlDataBean> tArrayList = liteOrm.query(
                    new QueryBuilder<>(CollectionSqlDataBean.class).where("username = ?", username));
    ...
    private class CallbackCollectionRunnable implements Runnable{
        ...
        private OnQueryListener onQueryListener;
        private ArrayList<CollectionSqlDataBean> tArrayList;

        public CallbackCollectionRunnable(OnQueryListener onQueryListener, ArrayList<CollectionSqlDataBean> tArrayList) {
            this.onQueryListener = onQueryListener;
            this.tArrayList = tArrayList;
        }
